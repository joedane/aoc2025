...
impl Range {
    ...
    fn check_invalid_part2(id: u64) -> bool {
        ...
        'digits: while part_digits > 0 {  
            let mut num_parts = total_digits / part_digits;
    ...
    fn sum_invalid_ids_part1(&self) -> usize {
        (self.0..=self.1).filter(|n| Range::check_invalid_part1(n)).map(|v| v as usize).sum()
    }
    fn sum_invalid_ids_part2(&self) -> usize {
        (self.0..=self.1).filter(|n| Range::check_invalid_part2(*n)).map(|v| v as usize).sum()
    }
}
